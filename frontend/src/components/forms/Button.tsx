...
const buttonVariants = cva( 'relative inline-flex items-center justify-center gap-2 font-semibold transition-all duration-fast focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-neural focus-visible:ring-offset-2 focus-visible:ring-offset-background-primary disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden active:scale-[0.98]',
  {
    variants: {
      ...
      size: { sm: 'h-9 px-4 text-sm rounded-lg', md: 'h-10 px-5 text-base rounded-xl' },
    },
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, ...props }, ref) => {
  const createRipple = (e: React.MouseEvent<HTMLButtonElement>) => {
    const button = e.currentTarget; const ripple = document.createElement('span'); const rect = button.getBoundingClientRect();
    ripple.className = 'absolute rounded-full bg-white/30 animate-ripple';
    ripple.style.left = ${e.clientX - rect.left - 10}px; ripple.style.top = ${e.clientY - rect.top - 10}px;
    ripple.style.width = ripple.style.height = '20px';
    button.appendChild(ripple);
    setTimeout(() => ripple.remove(), 600);
  };
  return ( <button ref={ref} className={cn(buttonVariants({ variant, size, className }))} onClick={(e) => { createRipple(e); props.onClick?.(e); }} {...props} /> );
});
